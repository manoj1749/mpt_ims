...
class _AddSupplierPageState extends ConsumerState<AddSupplierPage> {
  ...
  // Add controllers for GST fields
  ...
  final TextEditingController _customGstRateController =
      TextEditingController();
  final TextEditingController _customIgstRateController =
      TextEditingController();
  ...
  // Build GST Number from State Code + PAN + Suffix
  ...
  void _onStateCodeChanged(String value) {
    ...
    setState(() {
      if (value.isEmpty) {
        ...
        _filteredStates = states
            .where(
                (s) => s.stateCode.toLowerCase().contains(value.toLowerCase()))
            .take(5)
            .toList();
        _showStateSuggestions = true;
      }
    ...
    // Update GST number when state code changes
    _updateGstNumber();
  }
  ...
  void _selectState(StateModel stateModel) {
    ...
    // Update GST number when state is selected
    _updateGstNumber();
    _enforceTaxVisibility();
  }
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              _buildGstTextField('Total GST %', _totalGstController,
                ...
                // Quality Rating (Read-only - calculated from inspections)
                Container(
                  padding: const EdgeInsets.all(16),
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          Tooltip(
                            message: 'Quality rating is automatically calculated based on inspection results',
