...
class CustomerScopeIncomingInspectionListPage extends ConsumerStatefulWidget {
  const CustomerScopeIncomingInspectionListPage({super.key});

  @override
  ConsumerState<CustomerScopeIncomingInspectionListPage> createState() =>
      _CustomerScopeIncomingInspectionListPageState();
}

class _CustomerScopeIncomingInspectionListPageState
    extends ConsumerState<CustomerScopeIncomingInspectionListPage> {
  ...
  Widget _buildSummaryCards(List<QualityInspection> inspections) {
    ...
    int pendingInspections = inspections
        .where((inspection) => inspection.status == 'Pending' || inspection.status == 'Pending - Parameters Changed')
        .length;
                  ...
                  Expanded(
                    child: DropdownButtonFormField<String>(
                      ...
                      items: const [
                        ...
                        DropdownMenuItem(
                            value: 'Pending - Parameters Changed', child: Text('Pending - Parameters Changed')),
                        DropdownMenuItem(
  ...
  List<PlutoRow> _getRows(List<QualityInspection> inspections) {
    ...
    for (var inspection in inspections) {
      // Apply status filter
      if (_selectedStatus != 'All' && inspection.status != _selectedStatus) {
        continue;
      }
      ...
      if (_startDate != null) {
        try {
          ...
          if (inspectionDate.isBefore(_startDate!)) continue;
        } catch (e) {
      ...
      if (_endDate != null) {
        try {
          final inspectionDate = DateFormat('dd/MM/yyyy').parse(inspection.inspectionDate);
          if (inspectionDate.isAfter(_endDate!)) continue;
        } catch (e) {
          // Skip if date parsing fails
          continue;
        }
  ...
  Widget build(BuildContext context) {
    final allInspections = ref.watch(qualityInspectionProvider);
    final allGRNs = ref.watch(storeInwardProvider);
    
    // Filter to show only inspections for customer scope GRNs
    final inspections = allInspections.where((inspection) {
      final grn = allGRNs.firstWhere(
        (g) => g.grnNo == inspection.grnNo,
        orElse: () => allGRNs.first,
      );
      return grn.isCustomerScope;
    }).toList();
    
    // Debug: Print all customer scope quality inspections when data is loaded
    if (inspections.isNotEmpty) {
      print('==== ALL Customer Scope Quality Inspections (from provider) ====');
      for (var inspection in inspections) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Customer Scope Incoming Inspection'),
        elevation: 0,
      ),
