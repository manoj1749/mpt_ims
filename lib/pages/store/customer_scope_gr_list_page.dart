...
import 'add_customer_scope_gr_page.dart';
import 'package:hive/hive.dart';

class CustomerScopeGRListPage extends ConsumerStatefulWidget {
  const CustomerScopeGRListPage({super.key});

  @override
  ConsumerState<CustomerScopeGRListPage> createState() =>
      _CustomerScopeGRListPageState();
}

class _CustomerScopeGRListPageState extends ConsumerState<CustomerScopeGRListPage> {
  ...
  void initState() {
    ...
    void printGRsWhenBoxOpen([int retries = 10]) async {
      if (Hive.isBoxOpen('store_inwards')) {
        ...
        print('==== ALL Customer Scope GRs in store_inwards box ====');
        for (var gr in box.values.where((g) => g.isCustomerScope)) {
          print(gr.toString());
        }
        print('==== END Customer Scope GRs ====');
      } else if (retries > 0) {
  ...
  Future<void> _navigateToAddInward() async {
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => const AddCustomerScopeGRPage(),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    if (storeInwards.isNotEmpty) {
      print('==== ALL Customer Scope GRs in store_inwards box (from provider) ====');
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Customer Scope GR List'),
      ),
