...
class InternalDeliveryChallanListPage extends ConsumerStatefulWidget {
  const InternalDeliveryChallanListPage({super.key});

  @override
  ConsumerState<InternalDeliveryChallanListPage> createState() =>
      _InternalDeliveryChallanListPageState();
}

class _InternalDeliveryChallanListPageState
    extends ConsumerState<InternalDeliveryChallanListPage> {
  ...
  void _refreshGridRows({
    ...
    final dcs = sourceDcs
        .where((dc) => dc.dcType == 'internal' && dc.internalFlow == 'outward')
        .toList();

    final rowsNow = _buildRows(dcs);
    _stateManager!
      ..removeAllRows()
      ..appendRows(rowsNow);
  }
  ...
  List<PlutoRow> _buildRows(List<DeliveryChallan> deliveryChallans) {
  ...
  Widget build(BuildContext context) {
    ...
    // Filter for internal outward delivery challans only
    ...
    final rows = _buildRows(deliveryChallans);

    return Scaffold(
      appBar: AppBar(
        title: const Text('Internal Delivery Challans - Outward'),
        actions: [
          IconButton(
