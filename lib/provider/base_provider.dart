...
abstract class BaseProvider<T> extends StateNotifier<List<T>> {
  ...
  Future<void> loadData() async {
    ...
    try {
      ...
      print(
          'Found ${querySnapshot.docs.length} documents in Firebase for $collectionName');
      ...
      if (mounted) {
        ...
        _isLoaded = true;
        print(
            'Successfully loaded ${firebaseData.length} items from Firebase for $collectionName');
      }
    } catch (e) {
      ...
      if (localData.isNotEmpty && mounted) {
        ...
        _isLoaded = true;
        print(
            'Loaded ${localData.length} items from local cache for $collectionName');
      } else {
  ...
  Future<void> update(T model) async {
    try {
      ...
      final currentIndex =
          state.indexWhere((item) => getModelId(item) == modelId);
      if (currentIndex != -1) {
      ...
      final existingIndex = box.values
          .toList()
          .indexWhere((item) => getModelId(item) == getModelId(model));
      if (existingIndex != -1) {
  ...
  Future<bool> delete(T model) async {
    try {
      ...
      final existingIndex =
          box.values.toList().indexWhere((item) => getModelId(item) == modelId);
      if (existingIndex != -1) {
        await box.deleteAt(existingIndex);
      }
